        ...
        class BoxCastTest : ITestNonAlloc<RaySensor2D, RaycastHit2D> {
            public int Test(RaySensor2D sensor, RaycastHit2D[] results) {
                ...
                if (sensor.isSingleResult()) {
                    ...
                    return Physics2D.BoxCast(sensor.transform.position, sensor.Box.HalfExtents, sensor.transform.eulerAngles.z, sensor.direction, filter, results, sensor.Length);
                }
            }
        }
        void DrawBoxGizmo() {

...
float3 P3D_GetClosestPosition_Edge(float3 a, float3 b, float3 p)
{
	float3 ab  = b - a;
	float  abd = dot(ab, ab);
...
float3 P3D_GetClosestPosition_Triangle(float3 a, float3 b, float3 c, float3 p)
{
	float3 ab = b - a;
	float3 bc = c - b;
	float3 ca = a - c;
...
float3 P3D_GetClosestPosition(float3 p)
{
#if P3D_LINE || P3D_LINE_CLIP
	return P3D_GetClosestPosition_Edge(_Position, _EndPosition, p);
#elif P3D_QUAD || P3D_QUAD_CLIP
	float3 a = P3D_GetClosestPosition_Triangle(_Position, _EndPosition, _Position2, p);
	float3 b = P3D_GetClosestPosition_Triangle(_EndPosition2, _Position2, _EndPosition, p);

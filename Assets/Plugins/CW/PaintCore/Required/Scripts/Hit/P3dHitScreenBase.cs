...
namespace PaintIn3D
{
	...
	public abstract class P3dHitScreenBase : MonoBehaviour
	{
		...
		{
			if (GetComponent<P3dPointerMouse>() == null)
			{
				requiredButtons &= ~ButtonTypes.LeftMouse;
				requiredButtons &= ~ButtonTypes.RightMouse;
				requiredButtons &= ~ButtonTypes.MiddleMouse;

				gameObject.AddComponent<P3dPointerMouse>();
			}

			if (GetComponent<P3dPointerTouch>() == null)
			{
				requiredButtons &= ~ButtonTypes.Touch;

				gameObject.AddComponent<P3dPointerTouch>();
			}

			if (GetComponent<P3dPointerPen>() == null)
			{
				gameObject.AddComponent<P3dPointerPen>();
			}
		}
#endif

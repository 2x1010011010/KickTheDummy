...
namespace PaintIn3D
{
	public static class P3dBlit
	{
		...
		public static void Blit(RenderTexture rendertexture, Mesh mesh, int submesh, Texture texture, P3dCoord coord)
		{
			if (cachedBlitSet == false)
			{
				cachedBlit    = P3dCommon.BuildMaterial("Hidden/Paint in 3D/Blit");
				cachedBlitSet = true;
			}
			...
			P3dCommon.Draw(cachedBlit, 0, mesh, Matrix4x4.identity, submesh, coord);
